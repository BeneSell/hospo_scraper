...
def compute_query(sport_name:string, checked_sports_categorie:list, sort_value:string):
    ...
    if(sport_name != ""):
        data = data[data[["link_to_text","sport_type"]].applymap(lambda x: sport_name.lower() in x.lower()).any(axis=1)]

    # categorie filter
    if(checked_sports_categorie):
        print(checked_sports_categorie)
        data = data[data["sport_type"].str.contains('|'.join(checked_sports_categorie), flags=re.IGNORECASE, regex=True)]
    ...
    data["last_changed"] = data["last_changed"].apply(lambda x: x.strftime(format="%d.%m.%Y"))
    ...
    return {"data": data, "categorie_can_used": json.dumps(categories_can_used), "new_description": json.dumps(new_description.strip())}
...
def sports(sport_name=""):
    ...
    checked_sports_categorie = [urllib.parse.unquote(x) for x in checked_sports_categorie]
    print(checked_sports_categorie)
    ...
    return {"data":render_template("index.html", 
                            ...
                            load_serach = False
                            ), "categorie_can_used": direct_query["categorie_can_used"], "new_description": json.dumps(direct_query["new_description"]) }, 200
