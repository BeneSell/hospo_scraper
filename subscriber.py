...
def serach_for_new_user(link_to_text):
    ...
    try:
        print([x for x in message_dicts if x["message"]["text"] == f"/start {link_to_text_for_telegram}"])
        new_user = [x for x in message_dicts if x["message"]["text"] == f"/start {link_to_text_for_telegram}"][-1]
    except:
        return "no Message"
...
def create_json(unique_link_to_texts:list, chat_list:list) -> dict:
    ...
    for link_to_text in unique_link_to_texts:
        result_dict[link_to_text] = [y["chat_id"] for y in chat_list if y["link_to_text"] == link_to_text]
...
def send_message_to_user_list(link_to_text):
    ...
    for x in user_list:
        ...
        send_message(x, f"Hey es gibt neue Infos zu {link_to_text}. :) schau es dir hier an https://hosposcraper.pythonanywhere.com/index/{link_to_text} ")
