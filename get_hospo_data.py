...
def render_template_for_each_row(row, length):
    row_as_tag = render_template("template_list.html", day = row["day"], start_time = row["start_time"], end_time = row["end_time"], location = row["location"], difficulty = row["difficulty"], extra_information_1 = row["extra_information_1"], extra_information_2 = row["extra_information_2"], extra_information_3 = row["extra_information_3"], extra_information_4 = row["extra_information_4"], count=row["counter"]+1, length=length)
    return row_as_tag
...
def pipe_tables(temp_soup_today):
    ...
    new_html = []
    new_html_as_string = ""
    new_html = uni_df.apply(lambda row: render_template_for_each_row(row, uni_df["day"].count()), axis=1)
    for x in new_html:
...
def start_piping(content_tag):
    ...
    if(not pd.isna(for_table_pipe)):
        ...
        piped_content.select_one("table").insert_after(BeautifulSoup(new_html, 'html.parser'))
...
def comparing_homepage_data(row, curr_database):
    ...
    if(True):
        ...
        temp_dict = {"text": temp_text,"piped_text": piped_text, "link_to_text": row["link_to_text"], "sport_type": row["sport_type_x"], "last_changed": datetime.today()  }
        return pd.Series(temp_dict)
    else:
        # so it dosent overwrite last changed info
        temp_dict_from_old_base = curr_database[curr_database["link_to_text"] == row["link_to_text"]].iloc[0].to_dict()
        return pd.Series(temp_dict_from_old_base)


def get_info_from_homepage(session:CachedSession, session_today:CachedSession, curr_database):
    ...
    content_today = session_today.get("https://hochschulsport-koeln.de/", headers=headers)
    ...
    headlines_today = ["/"+x.text.strip() for x in soup_today.select(".card .class-header")]
    df_content = pd.DataFrame({"text": soup.select(".card .card-body"),"link_to_text": headlines, "sport_type": "Startseite"})
    df_content_today = pd.DataFrame({"text_today": soup_today.select(".card .card-body"),"link_to_text": headlines_today, "sport_type": "Startseite"})
    df_to_work_on = pd.merge(how="outer", on="link_to_text",left=df_content, right=df_content_today)
    return_df = df_to_work_on.apply(lambda row: comparing_homepage_data(row, curr_database), axis=1)
    return return_df
...
def cache_manipulation():
    ...
    for f in pathlib.Path.cwd().iterdir():
        if(re.match(".*cache.*sqlite",str(f)) and not f.is_dir() and not german_today in str(f)):
            cache_list.append({"file":f, "last_modified":f.lstat().st_mtime})
    df_cache_sqlite = pd.DataFrame(cache_list)
    last_modified_file = df_cache_sqlite.sort_values(by="last_modified",ascending=False).iloc[0]
    ...
    session_file_name = str(last_modified_file["file"]).split("\\")[-1].replace(".sqlite","")
...
def get_source():
    ...
    menu_parents = soup.select("li.menuparent")
    row_list = []
    ...
    for x in menu_parents:
        ...
        if(len(x.select("ul>li")) > 0):
            ...
            for y in x.select("ul>li"):
                ...
                temp_soup_today = BeautifulSoup(temp_today_response.content, 'html.parser')
                piped_text = ""
                
                hash_yesterday = hashlib.md5(temp_soup.select_one("div.region.region-content").decode_contents().encode('utf-8')).hexdigest()
                hash_today =  hashlib.md5(temp_soup_today.select_one("div.region.region-content").decode_contents().encode('utf-8')).hexdigest()
                has_changed = hash_yesterday != hash_today
                
                # if true it has changed
                has_endpoint_changed = curr_database[curr_database["link_to_text"] == endpoint]["link_to_text"].count() == 0
                ...
                if(has_changed or has_endpoint_changed):
                    for ele in temp_soup_today.findAll(attrs={"style" : True}):
                        del ele['style']
                    temp_text = temp_soup_today.select_one("div.region.region-content").decode_contents()
                    
                    piped_text = start_piping(temp_soup_today.select_one("div.region.region-content")).decode_contents()


                    temp_dict = {"text": temp_text,"piped_text": piped_text, "link_to_text": endpoint, "sport_type": sport_type, "last_changed": datetime.today()  }
                    ...
                    if(not has_endpoint_changed):
                     ...
                     if(curr_database[curr_database["link_to_text"] == endpoint]["link_to_text"].count() == 0):
                        # endpoint changed
                        pass
                     temp_dict_from_old_base = curr_database[(curr_database["link_to_text"] == endpoint)].iloc[0].to_dict()
